...
#open_ds <- open_dataset("data/supply_partitioned", format = "parquet")
...
# demand_ds <- open_dataset("data/demand_partitioned", format = "parquet")
# Tab 3 table source (CSV summary for CZs)

# ADD: DuckDB read-only connections
...
# Clean disconnect when the app stops
onStop(function() {
...
server <- function(input, output, session) {
  ...
  # Note: Do not sync leader selection back into the search UI; mirror Tab 3 behavior
  ...
  # This reactive should be removed or simplified since we primarily use CSV data
  demand_year_data <- reactive({
    # This is a fallback that may not be used if CSV data is available
    data.frame(
  ...
  observe({
    if (!is.null(cz_table_df) && nrow(cz_table_df) > 0) {
      ...
      if (length(match_idx) > 0) {
        ...
        # Build leader lists from demand_ds (collect before summarise to avoid Arrow limits)
        prettify_cz <- function(x) gsub("^\\d+ ", "", gsub(" CZ$", "", x))
        leaders_src <- demand_ds %>%
          filter(year != 2025) %>%
  ...
  output$demand_cz_trend <- renderPlot({
    ...
    # National averages mapped to metric
    # Align national averages to the same year range
