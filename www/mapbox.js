...
function updateCZLegend(metric) {
  ...
  // CHANGE: Added a formatting function to render numbers as strings with thousand separators (for display purposes only).
  ...
  if (isPercent) {
    // CHANGE: Percentage logic converted to universal syntax (automatically generated by breaks)
    // Bin 1: < first break
    html += `
      <div>
    ...
    // Middle bins: [breaks[i], breaks[i+1])
    ...
    // CHANGE: In numerical logic, apply fmt(...) with a thousand-separator comma to all values.
